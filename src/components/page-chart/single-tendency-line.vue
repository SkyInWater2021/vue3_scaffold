...
const axisData = computed(() => {
  ...
  const yData = (props.tableData ?? []).map(item => {
    if (props.type === "singleCount") {
      return item[props.columnLabel]?.count ?? null
    }

    if (props.type === "singleSize") {
      return item[props.columnLabel]?.size ?? null
    }
  })
  ...
  const finalOption: EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: (p: any) => {
        ...
        let target = date
        if (props.type === "singleCount") target = date + count
        if (props.type === "singleSize") target = date + byteSize
        return target
      },
    },
    xAxis: {
