...
// 定义多个区间，并按左端点从小到大排序,区间为左闭右开
export const colorMap: Map<RegExp[], ColorMapItem[]> = new Map()
 ...
 * @returns 图例配置
 */
export function getLegendColorMap(targetString = "") {
  if (!targetString) return []
  const mapKey = getTargetColorMapKey(targetString) ?? []
  const colorInfo = colorMap.get(mapKey) ?? []
 ...
 * @returns 查找到的区间项
 */
export function findInterval(val: number, targetString: string): ColorMapItem | null {
  const mapKey = getTargetColorMapKey(targetString) ?? []
  const colorInfo = colorMap.get(mapKey) ?? []
  let left = 0
 ...
 */
export function getTargetColorMapKey(targetString: string) {
  if (!targetString) return
