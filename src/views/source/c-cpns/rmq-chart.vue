...
import type { RMQLineResponseItem } from "@/services/system-status/types"
import { formatDate } from "@/utils"

import ChartLine from "../charts/rmq-line.vue"
...
const startTime = ref(
  import.meta.env.DEV
    ? new Date("2023-12-09 00:00:00").getTime()
...
const fetchParams = reactive({
  ...
  hostId: props.payload.pkId,
  interval: intervals[0].value,
  interval_unit: "hours",
  linesType: types[0].value,
  queueName: props.payload.queueName,
})

const responseData = ref<RMQLineResponseItem[]>()
const chartData = computed(() => {
  ...
  ;(responseData.value ?? []).forEach(item => {
    const timeStamp = item.periodDate
...
function fetchData() {
  ...
  systemStatusFRApi
    .queryThirdRmqLineChart({ ...fetchParams })
    .then(res => {
      if (res.code === 200) {
...
<template>
  <div class="">
    ...
    <div v-loading="fetchLoading" class="mt-5 h-[50vh]">
