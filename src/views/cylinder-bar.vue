...
import { BaseChart, echarts, type ECOption, type InitEchart } from "@/base-ui"
...
const options = computed(() => {
  ...
  const tooltip: ECOption["tooltip"] = {
    ...
    formatter: e => {
      ...
      const line1 = `<div>日期：${dateFormat(date ?? "", "YYYY-MM-DD HH") ?? "-"}</div>`
  ...
  const yAxis: ECOption["yAxis"] = [props.data?.barUnit, props.data?.lineUnit].map(
    (unit, index) => ({
      name: unit,
  ...
  const cylinderBarSeries: ECOption["series"] = [
    // 上圆柱
    {
      name: "文件大小",
      type: "pictorialBar",
...
<template>
  <BaseChart height="500px" :option="options" @completed="getChartInstance" />
</template>
