...
import type { RMQListResponse } from "@/services/system-status/types"
import { dateOffset } from "@/utils"

import { RMQTableConfig } from "../config/table"

const emit = defineEmits(["tagAdd"])

const currentTableConfig = reactive(cloneDeep(RMQTableConfig))

const fetchTime = ref("")
const tableContainerRef = ref<HTMLDivElement>()
const tableRef = ref<InstanceType<typeof BaseTable>>()
const tableResponseData = ref<RMQListResponse["list"]>()
...
function fetchListData() {
  ...
  systemStatusFRApi
    .queryThirdRmqColonyLists({ limit: pageSize, page: currentPage })
    .then(res => {
      if (res?.code === 200) {
...
function handleTagAdd(rowData: RMQListResponse["list"][0]) {
